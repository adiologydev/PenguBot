...
module.exports = class extends Route {
    ...
    async post(request, response) {
        ...
        if (body.data.relationships.currently_entitled_rewards.data.length > 0) {
            ...
            await this.client.settings.update("patrons.users", user)
                .catch(async e => {
                    await me.send(`<:patreon:502495572721270791> ***Client Update Error:*** ${patron.attributes.email}\n\n**Error:**\n${e}`);
                    return response.end(421);
                });
            return me.send(`<:patreon:502495572721270791> ***Gave Rewards To: \`${patron.attributes.email}\` | User: \`${user.id}\`***`);
        } else {
            const me = await this.client.users.fetch("136549806079344640").catch(() => null);
