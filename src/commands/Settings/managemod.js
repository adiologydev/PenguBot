...
module.exports = class extends Command {
    ...
    async add(msg, [memberOrRole]) {
        ...
        if (type === "member") {
            if (msg.guild.settings.users.mod.includes(memberOrRole)) return msg.sendMessage(`${this.client.emotes.cross} ***That user is already a Moderator, try another user or removing them first.***`);
        ...
        if (type === "role") {
            if (msg.guild.settings.roles.mod === memberOrRole.id) return msg.sendMessage(`${this.client.emotes.cross} ***That role is already a Moderator, try another role or removing it first.***`);
    ...
    async remove(msg, [memberOrRole]) {
        ...
        const type = memberOrRole instanceof Role ? "role" : "member";
        if (type === "member") {
            if (!msg.guild.settings.users.mod.includes(memberOrRole.id)) return msg.sendMessage(`${this.client.emotes.cross} ***That user is not a Moderator, try another user or adding them first.***`);
            const { errors } = await msg.guild.settings.update("users.mod", memberOrRole.id);
            if (errors.length) return msg.sendMessage(`${this.client.emotes.cross} ***There was an error: \`${errors[0]}\`***`);
            return msg.sendMessage(`${this.client.emotes.check} ***${memberOrRole} has been removed from Moderator.***`);
        }
        if (type === "role") {
            if (msg.guild.settings.roles.mod !== memberOrRole.id) return msg.sendMessage(`${this.client.emotes.cross} ***That role is already a Moderator, try another role or adding it first.***`);
