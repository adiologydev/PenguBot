...
module.exports = class extends Command {
    ...
    async run(msg, [Snowflakes = 1]) {
        ...
        if (Mone === Mtwo || Mone === Mthree || Mthree === Mtwo) {
            ...
            const winFlakes = msg.author.settings.snowflakes + Snowflakes + flakesPercent;
            ...
            return msg.sendMessage(`***${msg.author} You just won ❄ \`${flakesPercent}\`, you now have ❄ \`${msg.author.settings.snowflakes}\`! Good job!***`, { embed: embed });
        }
        const embed = new MessageEmbed()
            .setFooter("© PenguBot.com")
            .setTimestamp()
            .setDescription(`${Tone} | ${Ttwo} | ${Tthree}\n${Mone} | ${Mtwo} | ${Mthree}\n${Bone} | ${Btwo} | ${Bthree}`)
            .setColor("#d32f2f");
        await msg.author.settings.update("snowflakes", msg.author.settings.snowflakes - Snowflakes);
        return msg.sendMessage(`***${msg.author} You lost ❄ \`${Snowflakes}\`, you now have ❄ \`${msg.author.settings.snowflakes}\`! Better luck next time!***`, { embed: embed });
    }
