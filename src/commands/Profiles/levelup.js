...
module.exports = class extends Command {
    ...
    async toggle(msg) {
        if (msg.guild.settings.get("toggles.levelup")) {
    ...
    async type(msg, [type]) {
        ...
        if (type.match(/global/i)) {
            if (msg.guild.settings.get("misc.leveluptype") === "user") throw `${this.client.emotes.check} **Global** type of Level Ups are already set, try **guild** instead.`;
            ...
            if (msg.guild.settings.get("misc.leveluptype") === "guild") throw `${this.client.emotes.check} **Guild** type of Level Ups are already set, try **global** instead.`;
