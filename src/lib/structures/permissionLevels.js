...
module.exports = new PermissionLevels()
    ...
    .add(2, ({ member, guild }) => {
        ...
        return member.roles.cache.has(guild.settings.get("roles.dj")) || guild.settings.get("users.dj").includes(member.id);
    }, { fetch: true })

    // Pengu Staff
    .add(3, ({ member, guild }) => {
        if (!guild || !member) return false;
        return member.roles.cache.has(guild.settings.get("roles.staff")) || guild.settings.get("users.staff").includes(member.id);
    }, { fetch: true })
