...
const pLevels = new PermissionLevels()
    ...
    .add(0, () => true)
    // Pengu DJ
    .add(3, (c, m) => m.guild && m.guildConfigs["permissions.dj"].includes(m.member.id), { fetch: true })
    // Member is a PenguBot Moderator in the guild
    .add(4, (c, m) => m.guild && m.guildConfigs.permissions.mods.includes(m.member.id), { fetch: true })
