...
/**
 * A function to get a object from the genius api
 * @param {string} path A song to get
 * @returns {Object}
 */
...
/**
 * A function that uses cheerio to scrape results from a given url
 * @param {string} url The URL to scrape lyrics from
 * @returns {string}
 */
 ...
 * @returns {Object}
 */
Util.lyricsFormatter = async (req) => {
    ...
    const lyricsbody = await Util.scrapeLyrics(lyricdata.result.url);
