...
module.exports = new PermissionLevels()
    ...
    .add(0, () => true)
    // Pengu DJ
    .add(3, (c, m) => m.guild && m.guild.configs.permissions.dj.includes(m.member.id), { fetch: true })
    // Member is a PenguBot Moderator in the guild
    .add(4, (c, m) => m.guild && m.guild.configs.permissions.mods.includes(m.member.id), { fetch: true })
    // Member must have Kick/Ban Permissions
    .add(5, (c, m) => m.guild && m.member.permissions.has("BAN_MEMBERS") && m.member.permissions.has("KICK_MEMBERS") || m.guildConfigs.permissions.mods.includes(m.member.id), { fetch: true }) // eslint-disable-line
    // Member must have 'MANAGE_GUILD' or 'ADMINISTRATOR' permissions
    .add(6, (c, m) => m.guild && m.member.permissions.has("MANAGE_GUILD") || m.member.permissions.has("ADMINISTRATOR") || m.guildConfigs.permissions.admins.includes(m.member.id), { fetch: true }) // eslint-disable-line
