...
module.exports = class extends Command {
    ...
    async run(msg, [member]) {
        if (await msg.guild.configs.get("staff-mods").indexOf(member.id) !== -1) {
            await msg.guild.configs.update("staff-mods", member.id, { action: "remove" });
            msg.channel.send(`<:penguCross:432966551746904071> ***${member.tag} ${msg.language.get("MESSAGE_MOD_REMOVE")}***`);
        } else {
            await msg.guild.configs.update("staff-mods", member.id, { action: "add" });
            msg.channel.send(`<:penguCheck1:431440099675209738> ***${member.tag} ${msg.language.get("MESSAGE_MOD_ADD")}***`);
        }
    }
